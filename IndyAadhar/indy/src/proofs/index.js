...
exports.getProofRequests = async function(force) {
    if(force || !proofRequests) {
        ...
        let credentialCredDef = await indy.issuer.getCredDefByTag('MyCredential');
        if(credentialCredDef) {
            proofRequests['Credential-Data'] = {
                name: 'Credential-Data',
                version: '0.1',
                requested_attributes: {
                    'attr1_referent': {
                        'name': 'degree',
                        'restrictions': [{'cred_def_id': credentialCredDef.id}]
                    },
                    'attr2_referent': {
                        'name': 'status',
                        'restrictions': [{'cred_def_id': credentialCredDef.id}]
